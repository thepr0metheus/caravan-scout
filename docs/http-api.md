...
| `/api/state` | The machine: host identity/IP, GPUs, CPU/RAM, compute apps, heartbeat status, llama.cpp build and update status (`llamaUpdate`), the scout's own version (`scoutVersion`), per-cell `llamaNodes` (a cell that crashed since it was last started by hand carries `crash: {count, at, reason, gaveUp?}`, 2.5+), and `autostart` — the ports that start with the machine, stopped ones too (2.4+). The heartbeat pushes the same facts under the same names. |
