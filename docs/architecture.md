...
| `engines.py` | `ForeignEngines` — the model engines on this machine that are not its cells, rescanned every 10 s on a thread of their own; `EngineKind` → `Ollama`, `LmStudio` — the kinds as a table: default port, process names, how the API is read; `EngineAsk` — a GET on an engine's port (the scan's only verb: looking never changes an engine); `EngineCall` — a POST (or DELETE), used only by `ForeignEngines.act` and a download; `EngineStream` — a POST whose answer is read line by line (Ollama's `/api/pull`, 2.17); a model unloaded (2.14) or deleted (2.17) because the board asked, on a thread of its own, with what is under way and what failed last shown on the model; no model is loaded from here (2.18: the load, its idle limit and the memory estimate went — a cell in the engine loads its model when it starts); `LmsCli` — LM Studio's own command line (`~/.lmstudio/bin/lms`), for what its REST API does not say or do: starting and stopping its daemon and server, the limits of what is loaded (`lms ps --json`, 2.15); most of `lms` wakes LM Studio up when it is not running, so a read asks `lms server status` (which does not) first, and every sequence of commands holds the kind's lock — a read cannot meet a stop half-way (2.16.1) |
